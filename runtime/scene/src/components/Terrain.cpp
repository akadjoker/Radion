                   ...
                   f32 uvTiles)
{
    ...
    mHeight = height;
    mCellSize = cellSize;
    mHeightScale = Math::max(heightScale, 0.0001f);
    mUvTiles = uvTiles;
    ...
    mMaxLod = Math::min(maxLod, static_cast<u32>(indices.lods.size()));
...
bool Terrain::saveHeightmap(const char* filename)
{
    ...
    for (usize i = 0; i < mHeights.size(); ++i)
        image.pixels[i] =
            static_cast<u8>(Math::clamp(mHeights[i] / mHeightScale, 0.0f, 1.0f) * 255.0f + 0.5f);
...
{
    const f32 clamped = Math::max(tiles, 1.0f);
...
{
    const f32 clamped = Math::clamp(strength, 0.0f, 1.0f);
...
void Terrain::fillVegetation(VegetationChannel channel, f32 density)
{
    ...
    const u8 value = static_cast<u8>(Math::clamp(density, 0.0f, 1.0f) * 255.0f + 0.5f);
                              ...
                              VegetationChannel channel, f32 strength, bool erase)
{
    ...
    const Math::vec3 center = Math::vec3(
        Math::inverse(owner()->globalTransform()) * Math::vec4(worldCenter, 1.0f));
    ...
    const s32 minX = Math::clamp(static_cast<s32>(std::floor((center.x - radius + halfX) * pixelScaleX)),
                                0, mVegetationMask->width - 1);
    const s32 maxX = Math::clamp(static_cast<s32>(std::ceil((center.x + radius + halfX) * pixelScaleX)),
                                0, mVegetationMask->width - 1);
    const s32 minZ = Math::clamp(static_cast<s32>(std::floor((center.z - radius + halfZ) * pixelScaleZ)),
                                0, mVegetationMask->height - 1);
    const s32 maxZ = Math::clamp(static_cast<s32>(std::ceil((center.z + radius + halfZ) * pixelScaleZ)),
                                0, mVegetationMask->height - 1);
    ...
    for (s32 z = minZ; z <= maxZ; ++z)
        for (s32 x = minX; x <= maxX; ++x)
        {
            ...
            const f32 distance = Math::length(Math::vec2(localX - center.x, localZ - center.z));
            if (distance > radius)
                continue;
            const f32 falloff = 1.0f - Math::smoothstep(0.0f, radius, distance);
            ...
            const f32 delta = Math::clamp(strength * falloff, 0.0f, 1.0f);
            const f32 newValue = erase ? Math::max(0.0f, oldValue - delta)
                                       : Math::min(1.0f, oldValue + delta);
...
f32 Terrain::vegetationDensity(f32 localX, f32 localZ, VegetationChannel channel) const
{
    ...
    const u32 x1 = Math::min(x0 + 1, static_cast<u32>(mVegetationMask->width - 1));
    const u32 z1 = Math::min(z0 + 1, static_cast<u32>(mVegetationMask->height - 1));
    const u32 component = static_cast<u32>(channel);
    ...
    const f32 top = Math::mix(sample(x0, z0), sample(x1, z0), px - x0);
    const f32 bottom = Math::mix(sample(x0, z1), sample(x1, z1), px - x0);
    return Math::mix(top, bottom, pz - z0);
}

Math::vec4 Terrain::automaticSurfaceWeights(f32 localX, f32 localZ) const
{
    const f32 height01 = Math::clamp(heightAt(localX, localZ) / mHeightScale, 0.0f, 1.0f);
    const f32 slope = 1.0f - Math::clamp(normalAt(localX, localZ).y, 0.0f, 1.0f);
    const Math::vec4& thresholds = layerThresholds();
    const f32 lowEnd = Math::clamp(thresholds.x, 0.001f, 0.999f);
    const f32 highStart = Math::clamp(thresholds.y, 0.001f, 0.999f);
    const f32 slopeStart = Math::clamp(thresholds.z, 0.0f, 0.999f);
    const f32 slopeEnd = Math::max(slopeStart + 0.001f, thresholds.w);
    const f32 rock = Math::smoothstep(slopeStart, slopeEnd, slope);
    const f32 low = (1.0f - Math::smoothstep(0.0f, lowEnd, height01)) * (1.0f - rock);
    const f32 high = Math::smoothstep(highStart, 1.0f, height01) * (1.0f - rock);
    const f32 base = Math::max(0.0f, 1.0f - rock - low - high);
    Math::vec4 weights(base, rock, low, high);
    return weights / Math::max(Math::dot(weights, Math::vec4(1.0f)), 0.0001f);
}
                           ...
                           f32 strength, bool restoreAutomatic)
{
    ...
    for (s32 z = minZ; z <= maxZ; ++z)
        for (s32 x = minX; x <= maxX; ++x)
        {
            ...
            oldValue /= 255.0f;
            oldValue /= Math::max(Math::dot(oldValue, Math::vec4(1.0f)), 0.0001f);
            const Math::vec4 desired = restoreAutomatic
                                          ? automaticSurfaceWeights(localX, localZ)
                                          : target;
            Math::vec4 value = Math::mix(oldValue, desired,
                                       Math::clamp(strength * falloff, 0.0f, 1.0f));
            value /= Math::max(Math::dot(value, Math::vec4(1.0f)), 0.0001f);
...
{
    x = Math::clamp(x, 0, static_cast<s32>(mWidth) - 1);
    z = Math::clamp(z, 0, static_cast<s32>(mHeight) - 1);
...
f32 Terrain::heightAt(f32 localX, f32 localZ) const
{
    ...
    if (gx < 0.0f || gz < 0.0f || gx > mWidth - 1 || gz > mHeight - 1)
        return 0.0f;
    const u32 x0 = Math::min(static_cast<u32>(gx), mWidth - 2);
    const u32 z0 = Math::min(static_cast<u32>(gz), mHeight - 2);
...
Math::vec3 Terrain::normalAt(f32 localX, f32 localZ) const
{
    ...
    return Math::normalize(Math::vec3(left - right, 2.0f * step, top - bottom));
}
...
bool Terrain::raycast(const Ray& worldRay, Math::vec3& worldHit) const
{
    if (!valid() || !owner())
        return false;
    const Math::mat4 inverse = Math::inverse(owner()->globalTransform());
    Ray ray;
    ray.origin = Math::vec3(inverse * Math::vec4(worldRay.origin, 1.0f));
    ray.direction = Math::normalize(Math::vec3(inverse * Math::vec4(worldRay.direction, 0.0f)));
    f32 enter = 0.0f;
    f32 leave = std::numeric_limits<f32>::max();
    for (u32 axis = 0; axis < 3; ++axis)
    {
        if (Math::abs(ray.direction[axis]) < 1e-8f)
        {
    ...
    const f32 halfX = static_cast<f32>(mWidth - 1) * mCellSize * 0.5f;
    const f32 halfZ = static_cast<f32>(mHeight - 1) * mCellSize * 0.5f;
    const f32 epsilon = Math::max(1e-5f, mCellSize * 1e-5f);
    const Math::vec3 start = ray.at(Math::min(leave, enter + epsilon));
    s32 cellX = Math::clamp(static_cast<s32>(std::floor((start.x + halfX) / mCellSize)), 0,
                           static_cast<s32>(mWidth) - 2);
    s32 cellZ = Math::clamp(static_cast<s32>(std::floor((start.z + halfZ) / mCellSize)), 0,
...
{
    return edit(center, radius, Math::clamp(strength, 0.0f, 1.0f), true);
}
...
bool Terrain::flatten(const Math::vec3& worldCenter, f32 radius, f32 targetHeight, f32 strength)
{
    ...
    const Math::vec3 center =
        Math::vec3(Math::inverse(owner()->globalTransform()) * Math::vec4(worldCenter, 1.0f));
    const f32 halfX = static_cast<f32>(mWidth - 1) * mCellSize * 0.5f;
    const f32 halfZ = static_cast<f32>(mHeight - 1) * mCellSize * 0.5f;
    const s32 minX =
        Math::clamp(static_cast<s32>(std::floor((center.x - radius + halfX) / mCellSize)), 0,
                   static_cast<s32>(mWidth) - 1);
    const s32 maxX = Math::clamp(static_cast<s32>(std::ceil((center.x + radius + halfX) / mCellSize)),
                                0, static_cast<s32>(mWidth) - 1);
    const s32 minZ =
        Math::clamp(static_cast<s32>(std::floor((center.z - radius + halfZ) / mCellSize)), 0,
                   static_cast<s32>(mHeight) - 1);
    const s32 maxZ = Math::clamp(static_cast<s32>(std::ceil((center.z + radius + halfZ) / mCellSize)),
    ...
    for (s32 z = minZ; z <= maxZ; ++z)
        for (s32 x = minX; x <= maxX; ++x)
        {
            ...
            mHeights[index] =
                Math::mix(mHeights[index], targetHeight, Math::clamp(strength * falloff, 0.0f, 1.0f));
            changed = true;
        }
    if (!changed)
        return false;
...
u32 Terrain::generateGrass(Grass& grass, bool clearExisting) const
{
    ...
    const f32 sizeX = static_cast<f32>(mWidth - 1) * mCellSize;
    const f32 sizeZ = static_cast<f32>(mHeight - 1) * mCellSize;
    f32 spacing = Math::max(settings.spacing, 0.05f);
    ...
    const Math::mat4 worldToGrass = Math::inverse(grass.owner()->globalTransform());
    const Math::mat3 normalToWorld = Math::transpose(Math::inverse(Math::mat3(terrainToWorld)));
    const Math::mat3 normalToGrass = Math::transpose(Math::mat3(grass.owner()->globalTransform()));
    const f32 cosSlope = Math::cos(Math::radians(Math::clamp(settings.maximumSlopeDegrees,
                                                          0.0f, 89.9f)));
    const f32 jitter = Math::clamp(settings.jitter, 0.0f, 1.0f);
    const f32 density = Math::clamp(settings.density, 0.0f, 1.0f);
    const f32 scaleMin = Math::max(0.001f, Math::min(settings.minimumScale, settings.maximumScale));
    const f32 scaleMax = Math::max(scaleMin, Math::max(settings.minimumScale, settings.maximumScale));
    const f32 halfX = sizeX * 0.5f;
    const f32 halfZ = sizeZ * 0.5f;
    ...
    for (u64 z = 0; z < rows && planted < settings.maximumInstances; ++z)
    {
        for (u64 x = 0; x < columns && planted < settings.maximumInstances; ++x)
        {
            ...
            const f32 localX = Math::clamp((static_cast<f32>(x) + 0.5f + ox) * spacing,
                                          0.0f, sizeX) - halfX;
            const f32 localZ = Math::clamp((static_cast<f32>(z) + 0.5f + oz) * spacing,
                                          0.0f, sizeZ) - halfZ;
            ...
            const Math::vec3 worldNormal = Math::normalize(normalToWorld * localNormal);
            const Math::vec3 grassNormal = Math::normalize(normalToGrass * worldNormal);
            const f32 scale = Math::mix(scaleMin, scaleMax, terrainRandom(key ^ 0xa511e9b3u));
...
u32 Terrain::generateTrees(Forest& forest, bool clearExisting) const
{
    ...
    const f32 sizeX = static_cast<f32>(mWidth - 1) * mCellSize;
    const f32 sizeZ = static_cast<f32>(mHeight - 1) * mCellSize;
    f32 spacing = Math::max(settings.spacing, 0.1f);
    u64 columns = static_cast<u64>(std::ceil(sizeX / spacing));
    u64 rows = static_cast<u64>(std::ceil(sizeZ / spacing));
    const u64 candidateBudget =
    ...
    for (u32 i = 0; i < forest.speciesCount(); ++i)
        totalWeight += Math::max(0.0f, forest.speciesWeight(i));

    const Math::mat4 terrainToWorld = owner()->globalTransform();
    const Math::mat4 worldToForest = Math::inverse(forest.owner()->globalTransform());
    const f32 cosSlope = Math::cos(Math::radians(Math::clamp(settings.maximumSlopeDegrees,
                                                          0.0f, 89.9f)));
    const f32 jitter = Math::clamp(settings.jitter, 0.0f, 1.0f);
    const f32 density = Math::clamp(settings.density, 0.0f, 1.0f);
    ...
    for (u64 z = 0; z < rows && planted < settings.maximumInstances; ++z)
    {
        for (u64 x = 0; x < columns && planted < settings.maximumInstances; ++x)
        {
            ...
            if (totalWeight > 0.0f)
            {
                ...
                for (u32 i = 0; i < forest.speciesCount(); ++i)
                {
                    pick -= Math::max(0.0f, forest.speciesWeight(i));
                    if (pick <= 0.0f)
                    {
                        species = i;
                        break;
            ...
            const f32 scale = Math::mix(scaleMin, scaleMax, terrainRandom(key ^ 0x9e3779b9u));
        ...
        for (s32 x = minX; x <= maxX; ++x)
        {
            ...
            if (smoothing)
            {
                ...
                for (s32 dz = -1; dz <= 1; ++dz)
                    ...
                    {
                        const s32 sx = Math::clamp(x + dx, 0, static_cast<s32>(mWidth) - 1);
                        const s32 sz = Math::clamp(z + dz, 0, static_cast<s32>(mHeight) - 1);
                        average += source[vertexIndex(static_cast<u32>(sx), static_cast<u32>(sz))];
                        ++samples;
                    }
                mHeights[index] = Math::mix(source[index], average / samples, amount * falloff);
            }
            else
...
void Terrain::rebuildChunksTouching(u32 minX, u32 minZ, u32 maxX, u32 maxZ)
{
    ...
    auto chunkIndexFor = [](s32 globalIndex, u32 chunkCount) {
        ...
        return static_cast<u32>(Math::clamp(c, 0, static_cast<s32>(chunkCount) - 1));
    };
    ...
    for (u32 lz = 0; lz < ChunkWidth; ++lz)
    {
        for (u32 lx = 0; lx < ChunkWidth; ++lx)
        {
            ...
            const u8 encodedHeight = static_cast<u8>(
                Math::clamp(height / mHeightScale, 0.0f, 1.0f) * 255.0f + 0.5f);
            const u8 encodedSlope = static_cast<u8>(
                Math::clamp(slopeAmount, 0.0f, 1.0f) * 255.0f + 0.5f);
...
{
    const TerrainIndices& indices = TerrainIndices::instance();
    const s32 maxLod = Math::min(static_cast<s32>(indices.lods.size()) - 1,
                                static_cast<s32>(mMaxLod) - 1);
    const f32 scale = Math::max(Math::length(Math::vec3(transform[0])),
                              Math::max(Math::length(Math::vec3(transform[1])),
                                       Math::length(Math::vec3(transform[2]))));
    ...
    const f32 distance =
        Math::max(0.0f, Math::distance(cameraPosition, centre) - radius);
    const f32 ratio = distance / chunkSpan;
    const s32 target = Math::clamp(
