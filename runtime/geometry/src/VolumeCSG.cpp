...
f32 SphereSource::sampleDensity(const Math::Vec3& p) const { return m_radius - glm::length(p - m_center); }
...
PlaneSource::PlaneSource(Math::Vec3 normal, f32 offset) : m_normal(glm::normalize(normal)), m_offset(offset) {}
f32 PlaneSource::sampleDensity(const Math::Vec3& p) const { return glm::dot(m_normal, p) + m_offset; }
Sample PlaneSource::sample(const Math::Vec3& p) const { return {m_normal, sampleDensity(p)}; }

BoxSource::BoxSource(Math::Vec3 center, Math::Vec3 halfExtents) : m_center(center), m_halfExtents(glm::max(halfExtents, Math::Vec3(0.0f))) {}
f32 BoxSource::sampleDensity(const Math::Vec3& p) const
{
    const Math::Vec3 q = glm::abs(p - m_center) - m_halfExtents;
    const Math::Vec3 outside = glm::max(q, Math::Vec3(0.0f));
