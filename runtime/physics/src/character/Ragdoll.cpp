...
Math::Quaternion rotationBetween(const Math::Vec3& from, const Math::Vec3& to)
{
    const f32 cosine = glm::clamp(glm::dot(from, to), -1.0f, 1.0f);
    if (cosine > 0.9999f)
        return Math::Quaternion(1.0f, 0.0f, 0.0f, 0.0f);
    if (cosine < -0.9999f)
        return glm::angleAxis(glm::pi<f32>(), Math::Vec3(1.0f, 0.0f, 0.0f));
    const Math::Vec3 axis = glm::cross(from, to);
    const f32 scale = std::sqrt((1.0f + cosine) * 2.0f);
    return glm::normalize(Math::Quaternion(scale * 0.5f, axis / scale));
}
...
void Ragdoll::activate(PhysicsWorld& world, const std::vector<Math::Mat4>& globalPose,
                       const std::vector<Radion::LocalPose>& localPose, const Math::Mat4& ownerWorld)
{
    ...
    for (usize i = 0; i < mParts.size(); ++i)
    {
        ...
        const Math::Vec3 axis = glm::length(segment) > 1.0e-5f ? segment / glm::length(segment)
                                                              : Math::Vec3(0.0f, 1.0f, 0.0f);
        const Math::Quaternion orientation = rotationBetween(Math::Vec3(0.0f, 1.0f, 0.0f), axis);
        ...
        case Part::ShapeKind::Box:
            p.box = BoxShape(Math::Vec3(glm::max(length * 0.32f, 0.05f),
    ...
    for (usize i = 0; i < mParts.size(); ++i)
    {
        ...
        {
            Math::Vec3 axis = glm::cross(segmentAxis[static_cast<usize>(p.parentPart)], segmentAxis[i]);
...
void Ragdoll::writePose(std::vector<Radion::LocalPose>& localPose) const
{
    ...
    const Math::Mat4 invOwner = glm::inverse(mOwnerWorld);
    ...
    for (usize i = 0; i < mParts.size(); ++i)
    {
        ...
        const Math::Mat4 local = glm::inverse(p.parentChainLocal) * glm::inverse(parentModel) * boneModel;
