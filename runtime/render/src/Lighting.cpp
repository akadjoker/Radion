...
bool Lighting::createAtlasTexture()
{
    destroyAtlasTexture();
    GPU& gpu = GPU::getSingleton();
    const u32 size = Math::max(mAtlas.settings.size, 1u);
                             ...
                             bool renderShadows)
{
    ...
    for (const ShadowTile& tile : mAtlas.tiles())
    {
        ...
        light.shadowFade = Math::smoothstep(0.0f, 0.25f, tile.importance);
        ...
        if (light.type == RenderLightType::Point)
        {
            ...
            for (u32 face = 0; face < 6; ++face)
            {
                const Math::mat4 viewProjection =
                    projection * Math::lookAt(light.position, light.position + kFaceDirection[face],
                                             kFaceUp[face]);
        ...
        else
        {
            ...
            const f32 fov = light.type == RenderLightType::Rectangle
                               ? Math::radians(120.0f)
                               : Math::radians(Math::clamp(outerDegrees * 2.0f, 5.0f, 170.0f));
            const Math::mat4 viewProjection =
                Math::perspective(fov, 1.0f, 0.05f, Math::max(0.5f, light.range)) *
                Math::lookAt(light.position, light.position + light.direction, up);
            mMatrices.push_back(viewProjection);
...
void Lighting::submitDecals(const DecalSystem& decals)
{
    ...
    for (u32 i = 0; i < total; ++i)
    {
        ...
        entity.direction = Math::normalize(decal.rotation * Math::vec3(0.0f, 0.0f, 1.0f));
        entity.coneAngleCos =
                    ...
                    u32 screenHeight)
{
    ...
    const u32 tileSlots = Math::max(1u, mTilesX * mTilesY) * 8;
    ...
    if (tiledActive)
    {
        ...
        CullingBlock block;
        block.inverseProjection = Math::inverse(frame.projection);
