                             ...
                             Math::vec3& normal)
{
    ...
    const f32 determinant = Math::dot(edge1, p);
    if (std::abs(determinant) < 1e-8f) return false;
    const f32 inverse = 1.0f / determinant;
    const Math::vec3 toOrigin = origin - triangle.a;
    const f32 u = inverse * Math::dot(toOrigin, p);
    if (u < 0.0f || u > 1.0f) return false;
    const Math::vec3 q = Math::cross(toOrigin, edge1);
    const f32 v = inverse * Math::dot(direction, q);
    if (v < 0.0f || u + v > 1.0f) return false;
    const f32 t = inverse * Math::dot(edge2, q);
...
bool TriangleBVH::build(const MeshData& mesh, const Math::mat4& transform)
{
    ...
    for (usize i = 0; i < mesh.indices.size(); i += 3)
    {
        ...
        if (Math::length(Math::cross(triangle.b - triangle.a, triangle.c - triangle.a)) > 1e-8f) triangles.push_back(triangle);
    }
...
{
    if (m_nodes.empty() || maxDistance <= 0.0f || Math::length(direction) < 1e-8f) return false;
    const Math::vec3 normalized = Math::normalize(direction);
