...
int main(int, char**)
{
    ...
    MeshData box;
    if (!Assets().buildMeshData(MeshDesc::box(glm::vec3(1.5f)), box))
    {
    ...
    while (engine.update())
    {
        const f32 deltaTime = glm::min(engine.getWindow().getDeltaTime(), 0.1f);
        ...
        const glm::vec3 cameraPos = glm::vec3(std::cos(orbit), 0.6f, std::sin(orbit)) * 4.0f;
        ...
        for (const glm::vec3& p : box.positions)
            batch.point(p, glm::vec4(1.0f, 0.85f, 0.1f, 1.0f));
        if (box.indices.size() >= 3)
        {
            batch.triangle(box.positions[box.indices[0]], box.positions[box.indices[1]],
                           box.positions[box.indices[2]], glm::vec4(1.0f, 0.15f, 0.15f, 0.5f));
        }
