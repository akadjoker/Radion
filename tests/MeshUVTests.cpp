...
void testNeighbourKeepsItsUVs()
{
    ...
    CHECK(near(mesh.uvs[4], Math::vec2(-0.5f, -0.5f)));
...
void testScaleKeepsTheCentre()
{
    ...
    for (usize i = 0; i < mesh.uvs.size(); ++i)
        mesh.uvs[i] += Math::vec2(10.0f, 10.0f);

    CHECK(assets.transformFaceUVs(mesh, {}, Math::vec2(3.0f), 0.0f, Math::vec2(0.0f)));

    Math::vec2 min = mesh.uvs[0];
    Math::vec2 max = mesh.uvs[0];
    for (usize i = 1; i < mesh.uvs.size(); ++i)
    {
        min = Math::min(min, mesh.uvs[i]);
...
void testOffsetAndRotation()
{
    ...
    CHECK(assets.transformFaceUVs(moved, {}, Math::vec2(1.0f), 0.0f, Math::vec2(0.25f, -0.5f)));
    CHECK(near(moved.uvs[0], Math::vec2(0.25f, -0.5f)));
    CHECK(near(moved.uvs[2], Math::vec2(1.25f, 0.5f)));

    // 90 degrees about the centre (0.5,0.5) takes the corner at (0,0) to
    // (1,0): x' = cx + (x-cx)cos - (y-cy)sin.
    MeshData turned = makeQuad();
    CHECK(assets.transformFaceUVs(turned, {}, Math::vec2(1.0f), 90.0f, Math::vec2(0.0f)));
    ...
    CHECK(near(full.uvs[0], Math::vec2(0.0f, 0.0f), 1e-3f));
    CHECK(near(full.uvs[2], Math::vec2(1.0f, 1.0f), 1e-3f));
}
...
void testRejectsSelectionOfNothing()
{
    AssetManager& assets = AssetManager::getSingleton();
    MeshData mesh = makeQuad();
    const std::vector<Math::vec2> before = mesh.uvs;

    CHECK(!assets.transformFaceUVs(mesh, {17, 99}, Math::vec2(2.0f), 0.0f, Math::vec2(0.0f)));
    CHECK(mesh.positions.size() == 4);
    for (usize i = 0; i < mesh.uvs.size(); ++i)
...
void testSplitCarriesEveryStream()
{
    ...
    mesh.uvs2.assign(4, Math::vec2(0.25f, 0.75f));
