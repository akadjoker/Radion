...
void testSignConvention()
{
    ...
    CHECK(glm::abs(mesh.sampleDensity(Math::Vec3(0.0f)) - 1.0f) < 0.01f);

    CHECK(mesh.sampleDensity(Math::Vec3(5.0f, 0.0f, 0.0f)) < 0.0f);
    CHECK(glm::abs(mesh.sampleDensity(Math::Vec3(5.0f, 0.0f, 0.0f)) + 4.0f) < 0.01f);
    ...
    CHECK(corner < 0.0f);
    CHECK(glm::abs(glm::abs(corner) - glm::length(Math::Vec3(1.0f))) < 0.01f);
}
...
void testOffCentreMesh()
{
    ...
    CHECK(mesh.sampleDensity(Math::Vec3(0.0f)) < 0.0f);
    CHECK(glm::abs(mesh.sampleDensity(Math::Vec3(10.0f, -5.0f, 3.0f)) - 1.0f) < 0.01f);
}
