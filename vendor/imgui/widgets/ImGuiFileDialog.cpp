...
void ImGuiFileDialog::BeginCreateFolder()
{
    mCreatingFolder = true;
    mNewFolderName = "New Folder";
    mFocusNewFolder = true;
}

void ImGuiFileDialog::ConfirmCreateFolder()
{
    if (!mNewFolderName.empty())
    {
        std::error_code ec;
        const std::filesystem::path new_folder = mCurrentDirectory / mNewFolderName;
        std::filesystem::create_directory(new_folder, ec);
        if (!ec)
        {
            mSelectedPath = new_folder;
        }
        else
        {
            Radion::Log::error("ImGuiFileDialog: could not create folder '%s' (%s)",
                               new_folder.string().c_str(), ec.message().c_str());
        }
    }
    mCreatingFolder = false;
...
void ImGuiFileDialog::CancelCreateFolder()
{
    mCreatingFolder = false;
    mNewFolderName.clear();
}
                             ...
                             const std::filesystem::path& bin_directory)
{
    ...
    ImGui::SetNextWindowSize(ImVec2(800.0f, 480.0f), ImGuiCond_FirstUseEver);
    if (!ImGui::BeginPopupModal(popup_title.c_str(), nullptr, ImGuiWindowFlags_NoCollapse))
    {
        return false;
    }
    ...
    {
        ImGui::TextWrapped("'%s' already exists.", mFileName.c_str());
        ImGui::TextWrapped("Overwrite it?");
        ImGui::Separator();
    ...
    if (mCreatingFolder)
    {
        ...
        if (mFocusNewFolder)
        {
            ...
            mFocusNewFolder = false;
        }
        const bool submitted = ImGui::InputText("##new_folder_name",
                                                 &mNewFolderName,
        ...
        ImGui::SameLine();
        if (ImGui::Button("Cancel##new_folder"))
        {
            CancelCreateFolder();
        }
    }
    ...
    const float reserved_bottom_height =
        (mMode != Mode::ChooseFolder ? ImGui::GetFrameHeightWithSpacing() : 0.0f) +
        ImGui::GetFrameHeightWithSpacing();
    const ImVec2 entries_size(ImGui::GetContentRegionAvail().x,
                              std::max(120.0f, ImGui::GetContentRegionAvail().y - reserved_bottom_height));
    ImGui::BeginChild("##file_dialog_entries", entries_size, true);
    ...
    if (mViewMode == ViewMode::Details)
    {
        if (ImGui::BeginTable("##file_dialog_table", 3, ImGuiTableFlags_RowBg | ImGuiTableFlags_BordersInnerV | ImGuiTableFlags_SizingStretchProp))
        {
            ...
            for (const Entry& entry : entries)
            {
                ...
                // Capture hover/visibility for the row's own Selectable
                // right away - RenderEntryLabel below submits its own Text
                // item, which would otherwise become the "current item"
                // IsItemHovered() queries, silently breaking double-click.
                const bool row_hovered = ImGui::IsItemHovered();
                const bool row_double_clicked = row_hovered && ImGui::IsMouseDoubleClicked(ImGuiMouseButton_Left);
    ...
    else
    {
        for (const Entry& entry : entries)
        {
            ...
            // See the Details-view branch above: capture hover before
            // RenderEntryLabel submits its own item and steals it.
            const bool row_hovered = ImGui::IsItemHovered();
            const bool row_double_clicked = row_hovered && ImGui::IsMouseDoubleClicked(ImGuiMouseButton_Left);
            if (ImGui::IsItemVisible())
            {
    ...
    if (ImGui::Button(ConfirmLabel(mMode).c_str(), ImVec2(100.0f, 0.0f)))
    {
        ...
        const std::filesystem::path target_path = (mCurrentDirectory / mFileName).lexically_normal();
        if (mMode == Mode::SaveFile && std::filesystem::exists(target_path, exists_error))
        {
